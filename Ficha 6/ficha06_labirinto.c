...
{
  int x = nz + MAZE_WIDTH/2.0 + 0.5;    
  int z = nx + MAZE_HEIGHT/2.0 + 0.5;
...
void desenhaModeloDir(Objeto obj,int width, int height)
{
  ...
  //desenha Seta
	glPushMatrix();
		glRotated(GRAUS(modelo.objeto.dir) - 90,0.0,0.0,1.0); 
		//Seta
		glColor3f(1,0.4,0.4);
		glBegin(GL_POLYGON);
			glVertex2d(0,25);
...
void desenhaLabirinto(GLuint texID)
{
  ...
  glPushMatrix();
    glTranslatef(-MAZE_HEIGHT*0.5,0.5,-MAZE_WIDTH*0.5);   //para centrar?
    for(i=0;i<MAZE_HEIGHT;i++)
      for(j=0;j<MAZE_WIDTH;j++)
        if(mazedata[i][j]=='*')         //desenha parede com as texturas
        {
          glPushMatrix();
            glTranslatef(i,0,j);
...
void motionNavigateSubwindow(int x, int y)
{
  int dif;
	...
	if(dif>0){ //olhar para a direita
		...
		/*
		if(estado.camera.dir_long<modelo.objeto.dir-RAD(45))
			estado.camera.dir_long=modelo.objeto.dir-RAD(45);
		*/
	}
...
void setNavigateSubwindowCamera(Camera *cam, Objeto obj)
{
  ...
  {
    cam->eye.x=obj.pos.x;
	  cam->eye.y=obj.pos.y + 0.8;     //camara +/- altura da cabeça
    cam->eye.z=obj.pos.z;
...
void timer(int value)
{
  ...
  if(estado.teclas.up) {
    ...
    nx = modelo.objeto.pos.x + (velocidade * cos(modelo.objeto.dir));
    nz = modelo.objeto.pos.z + (velocidade * -sin(modelo.objeto.dir));
  ...
  if(estado.teclas.left) {
    ...
    modelo.objeto.dir += 5 * velocidade;
